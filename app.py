...
def analyze_stock(df, name, current, symbol, money_flow):
    ...
    if back_yellow and shrink and (brother_yellow >= yellow_prev * 0.997) and safe_get('MA60', 0) >= df['MA60'].shift(1).iloc[-1] if 'MA60' in df else False and safe_get('近期振幅', 0) >= 11.9 and safe_get('远期振幅', 0) >= 19.5:
        signals['浩哥黄线战法'] = True
