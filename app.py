...
# 浩哥战法核心逻辑（简化版，无市值）
# ======================
def analyze_stock(df, name, current, news, lhb_net):
    ...
    # 浩哥生动评论
    comment = f"浩哥瞅了瞅 {name}，当前价 {current:.2f} 元。"
...
if st.button("让浩哥分析"):
    ...
    for symbol in codes:
        ...
        total_score, comment, buy_advice, news_text = analyze_stock(None, stock_name, current, 0, news, lhb_net)
...
st.sidebar.info("浩哥亲自点评，实时价格 + 最新新闻，评论生动接地气。分享给朋友们用吧！")
